...
fn number_edge_of_symbol(symbol: Symbol, number: Number) -> bool {
    let y = number.begin.1;
    for x in number.begin.0..=number.end.0 {
        if (x >= symbol.coord.0 - 1 && x <= symbol.coord.0 + 1)
            && (y <= symbol.coord.1 + 1 && y >= symbol.coord.1 - 1)
        {
            return true;
        }
    }
...
pub fn process_part_one(input: &str) -> isize {
    let (coords, numbers) = parse_input(input);
    let mut total = 0;

    for coord in coords {
        total += numbers
            .iter()
            .filter(|n| number_edge_of_symbol(coord.clone(), **n))
            .map(|n| n.value)
            .sum::<isize>();
    }
...
pub fn process_part_two(input: &str) -> isize {
    let (coords, numbers) = parse_input(input);
    let mut total = 0;

    for coord in coords {
        if coord.value == "*" {
            let gear_nums: Vec<isize> = numbers
                .iter()
                .filter(|n| number_edge_of_symbol(coord.clone(), **n))
                .map(|n| n.value)
                .collect();
            ...
            if gear_nums.len() == 2 {
                total += gear_nums.iter().product::<isize>();
            }
        }
    }
